...
/* crear trigger que descuente el stock cuando en el 
   detalle especifico la cantidad de producto */
   
create or replace trigger tgr_DescontarStockProductos   
...
insert into DetallePedidos values ( 11077 , 61 , 16.00 , 60 );
...
select * 
from Productos
where not EXISTS (select precio from Productos where Precio > 10);

select * 
from Productos
where  EXISTS (select precio from Productos where Precio > 10);


-- insert all

INSERT ALL
 into categorias (idcategoria,nombre) values (10,'CCCCCCCC')
 into categorias (idcategoria,nombre) values (11,'YYYYYYYY')
 into categorias (idcategoria,nombre) values (12,'OOOOOOOO')
 into categorias (idcategoria,nombre) values (13,'MMMMMMMM')
 into categorias (idcategoria,nombre) values (14,'AAAAAAAA')
 into categorias (idcategoria,nombre) values (15,'RRRRRRRR')
select * from Dual;
...
-- clausula pivot
select * from (
  select P.idCliente,DP.idProducto
  from Pedidos P full outer join DetallePedidos DP using (idPedido)
)pivot (
  count(idProducto)
  for idProducto in (10,20,30)
)
order by P.idCliente

select * from pedidos

-- clausula  REGEXP_LIKE
select *
from Clientes
where REGEXP_LIKE(NombreCliente,'^A(*)')


select *
from Clientes
where REGEXP_LIKE(NombreCliente,'(*)n$');
...
-- creacion de tablas foreign key
...
DROP CONSTRAINT fk_supplier ;
  ...
  ALTER TABLE products
DROP CONSTRAINT fk_supplier_comp ;
...
DROP CONSTRAINT check_supplier_id;
  ...
  ALTER TABLE suppliers
DROP CONSTRAINT check_supplier_name;

ALTER TABLE suppliers
ENABLE CONSTRAINT check_supplier_id;

ALTER TABLE suppliers
ENABLE CONSTRAINT check_supplier_name;

ALTER TABLE suppliers
DISABLE CONSTRAINT check_supplier_id;

ALTER TABLE suppliers
DISABLE CONSTRAINT check_supplier_name;
...
CREATE INDEX supplier_idx
  ON suppliers (supplier_name, city);
...
DROP INDEX supplier_index_name;
...
set serveroutput on;
declare
  var_idProducto productos.idproducto%type;
  var_Nombre productos.Nombre%type;
  var_Precio productos.Precio%type;
  cursor crs_Elementos (var_Precio_Param in number) 
  is select idProducto,Nombre,Precio   from Productos  where Precio = var_Precio_Param;
begin    
   OPEN crs_Elementos(10);
   FETCH crs_Elementos INTO var_idProducto,var_Nombre, var_Precio;
  dbms_output.put_line(var_idProducto||' - '||var_Nombre ||' - '|| var_Precio);
   CLOSE crs_Elementos;
end;
...
declare
  ...
  is select idProducto,Nombre,Precio   from Productos  where Precio > var_Precio_Param;
begin    
   for i in crs_Elementos(10) loop  
  dbms_output.put_line(i.idProducto||' - '||i.Nombre ||' - '|| i.Precio);
   end loop;
end;
...
-- tablas global temporales
  CREATE GLOBAL TEMPORARY TABLE suppliers
( supplier_id numeric(10) NOT NULL,
  supplier_name varchar2(50) NOT NULL,
  contact_name varchar2(50)
);
-- tablas local temporales
